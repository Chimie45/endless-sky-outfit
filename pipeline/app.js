...
function renderGalaxyMap(){
  ...
  const SYS=DATA.systems||{}; const names=Object.keys(SYS).filter(n=>Array.isArray(SYS[n].pos) && sysTier(n)<=state.tier);
  ...
  for(const n of names){ const a=SYS[n]; for(const ln of (a.links||[])){ const b=SYS[ln]; if(!b||!Array.isArray(b.pos)||sysTier(ln)>state.tier)continue; const k=n<ln?n+"|"+ln:ln+"|"+n; if(seen.has(k))continue; seen.add(k); lines+='<line x1="'+a.pos[0]+'" y1="'+a.pos[1]+'" x2="'+b.pos[0]+'" y2="'+b.pos[1]+'"/>'; } }
  let nodes="";
...
function renderSystemDetail(name){ const box=el("mapDetail"); if(!box) return; const s=(DATA.systems||{})[name]; if(!s){ box.innerHTML=""; return; }
  ...
  const links=(s.links||[]).filter(l=>(DATA.systems||{})[l] && sysTier(l)<=state.tier);
